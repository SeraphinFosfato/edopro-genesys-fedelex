...
static void LoadReplay() {
	...
	if(std::exchange(open_file, false)) {
		...
		if(!res || (replay.IsOldReplayMode() && (!mainGame->IsGameDataReady() || !replay.CanBePlayedInOldMode())))
			return;
	} else {
		...
		if(!replay.OpenReplay(path) || (replay.IsOldReplayMode() && (!mainGame->IsGameDataReady() || !replay.CanBePlayedInOldMode())))
			return;
	}
	...
	case irr::EET_GUI_EVENT: {
		...
		case irr::gui::EGET_LISTBOX_CHANGED: {
			...
			case LISTBOX_REPLAY_LIST: {
				...
				bool can_be_played = replay.CanBePlayedInStreamedMode() || (replay.CanBePlayedInOldMode() && mainGame->IsGameDataReady());
				...
				mainGame->chkYrp->setChecked(false);
				mainGame->chkYrp->setEnabled(replay.HasPlayableYrp() && mainGame->IsGameDataReady());
				break;
			}
			case LISTBOX_SINGLEPLAY_LIST: {
				...
				if(sel == -1)
					break;
				mainGame->btnLoadSinglePlay->setEnabled(mainGame->IsGameDataReady());
			...
			case irr::DROP_END:	{
				if(to_open_file.size()) {
					...
					if(extension == L"ydk" && isMenu && mainGame->deckBuilder.SetCurrentDeckFromFile(Utils::ToPathString(to_open_file))) {
						...
						mainGame->deckBuilder.Initialize();
						return true;
					} else if(mainGame->IsGameDataReady() && extension == L"lua" && !mainGame->wReplay->isVisible()) {
						open_file = true;
						...
						return true;
					} else if(mainGame->IsGameDataReady() && (extension == L"yrpx" || extension == L"yrp") && !mainGame->wSinglePlay->isVisible()) {
						open_file = true;
						open_file_name = Utils::ToPathString(to_open_file);
